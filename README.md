<h1 align="center">Hola, soy Omar Torres 👋</h1>

<p align="center">
  Desarrollador Frontend enfocado en <strong>accesibilidad</strong>, <strong>UI</strong> y desarrollo web moderno.
</p>

<p align="center">
  <a href="https://github.com/TU-USUARIO">
    <img src="https://img.shields.io/badge/GitHub-Perfil-181717?style=for-the-badge&logo=github">
  </a>
  <a href="https://www.linkedin.com/in/TU-LINKEDIN/">
    <img src="https://img.shields.io/badge/LinkedIn-Perfil-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white">
  </a>
  <a href="mailto:TU-CORREO">
    <img src="https://img.shields.io/badge/Email-Contacto-EA4335?style=for-the-badge&logo=gmail&logoColor=white">
  </a>
</p>

---

## 🚀 Sobre mí

- Egresado de Ingeniería en Informática
- Enfocado en desarrollo **Frontend**, accesibilidad web y experiencia de usuario
- Experiencia con interfaces modernas usando **Next.js**, **React**, **Tailwind CSS** y **TypeScript**
- También trabajo con **Django**, **Supabase** y **PostgreSQL**
- Interés en construir productos web claros, útiles y bien diseñados

---

## 🛠 Tecnologías

### Frontend
<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,ts,react,nextjs,tailwind,svelte" />
</p>

### Backend
<p>
  <img src="https://skillicons.dev/icons?i=python,django,supabase,postgres" />
</p>

### Herramientas
<p>
  <img src="https://skillicons.dev/icons?i=git,github,docker,vscode,vercel" />
</p>

---

## 📌 En lo que estoy trabajando

- Interfaces accesibles bajo buenas prácticas de **WCAG**
- Proyectos web con enfoque en **UI limpia y funcional**
- Sistemas con frontend moderno y backend escalable
- Mejora de experiencia de usuario en plataformas reales

---

## 💼 Proyectos destacados

### 🔹 NeuroGestión
Sitio y sistema con foco en accesibilidad cognitiva, diseño claro e interfaz usable para distintos perfiles de usuarios.

### 🔹 Cocinas Appel
Proyecto web con gestión de productos, panel administrativo, sistema de pedidos y experiencia de compra.

### 🔹 Proyectos con Django + Frontend moderno
Desarrollo de soluciones web conectando frontend moderno con backend robusto y base de datos.

---

## 🎯 Objetivo profesional

Seguir creciendo como desarrollador web, especializándome en:

- Frontend profesional
- Accesibilidad
- UI/UX
- Arquitectura de interfaces
- Desarrollo de productos digitales con impacto real

---

## 📊 Estadísticas

<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=TU-USUARIO&show_icons=true&theme=tokyonight&hide_border=true" />
  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=TU-USUARIO&layout=compact&theme=tokyonight&hide_border=true" />
</p>

---

## 📫 Contacto

- LinkedIn: [TU-LINKEDIN](https://www.linkedin.com/in/TU-LINKEDIN/)
- Email: **TU-CORREO**
- GitHub: [@TU-USUARIO](https://github.com/TU-USUARIO)

---

<p align="center">
  <em>Construyendo interfaces claras, accesibles y bien hechas.</em>
</p>
